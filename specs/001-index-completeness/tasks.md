...
- [x] T001 Confirm #284 is a sub-issue of #282 and `In Progress`; worktree `../hejbro-worktrees/phase10-index-completeness` on branch `phase10-index-completeness` (based on `phase10-speckit` until #285 merges, then rebase onto `dev`); `pnpm install` done (no code)
...
- [x] T033 [P] [US3] Unit: emit renders `((lower("app"."users"."email")))` — the expression's own parentheses plus the column list's (F7, owner decision 2026-08-22; see contracts/sql.md) — composes with unique + where; expression change → drop + create; also covers a non-function-call operator expression (`->>`), which cannot skip the wrap the way a bare function call could — packages/core/test/table-kind-emit.test.ts
...
- [x] T063 Roadmap: docs/plans/2026-08-22-0.2.0-roadmap.md frontier line + pilot verdict paragraph (ceremony cost, override quality, what to change)
...
- [x] T066 `/speckit-analyze` clean; `/speckit-converge` reports Converged (or its appended tasks are done)
