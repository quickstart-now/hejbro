...
- [ ] T006 Unit test (failing) then fix: `@hejbro/supabase` `indexDescription` renders an expression entry as `(<expression>)` and a named entry as today — packages/supabase/test/rls-cached-auth-outside-rls.test.ts, packages/supabase/src/validators/rls-cached-auth-outside-rls.ts (compile-driven by T003) (moved to US3 — nothing landed in Foundational)
