...
| `unique-index-method` | `IndexBuilder.on()` | `index "<name or <derived>>" is unique and uses "<m>" — Postgres supports unique only on btree indexes. Next: drop .unique() or drop .using("<m>").` |
