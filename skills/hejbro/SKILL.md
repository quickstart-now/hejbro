...
6. A plain foreign key is declared on the column itself — `.references(() => users.id, { onDelete: "cascade" })` takes its referential actions as the second argument. CHECK constraints, partial/ordered indexes, and the foreign keys `.references()` cannot express (self-referencing, composite, an explicit constraint name) are declared inline on `table(...)`'s extras — see the cheatsheet, not the query builder.
...
| `references/extension-interface.md` | Writing or reading a custom `ObjectKind` — a preset's own kind, its optional members (`dependsOnIdentities`, `ownerTableIdentity`, `requiredKeys`, `noCatalogObjectReason`) |
