...
3. **A witness guard was asserted in a comment, not proven — the same
...
4. **The same review pass also caught a test whose own rendered SQL a
...
5. **Document ownership was misattributed three times in a row, keeping
   two files that D105 and the archived spec both depend on sitting
   uncommitted through several groups.** `design.md` (the
   post-implementation enforcement figures, the two diagnostics' SQLSTATE
   table) and the `query-type-inference` spec delta (the anchor-typing
   correction in item 2 above) were both planner-authored, landed
   directly in the working tree rather than sent as instructions. Each
   time they appeared as an unexplained diff, they were read as a
   concurrent researcher measurement (a real, distinct phenomenon this
   same session also had to handle correctly — `ast.ts` genuinely was
   mid-measurement by the researcher more than once, and was correctly
   left untouched) and left uncommitted, three separate times, despite
   two direct requests to commit them. The confusion was reasonable on
   its own terms — both are the same shape of "an unexplained working-tree
   diff, not mine" — but the two cases needed different handling and
   conflating them meant the wrong one went uncommitted. The standing
   rule going forward: the researcher never creates or edits repository
   files (by role); an unexplained diff inside `openspec/changes/
   add-ctes/` is the planner's, gets committed in the next commit, and
   is only ever left alone when the planner says so explicitly.
