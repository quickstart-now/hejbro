...
<a id="w12"></a>
## W12 — D106 round 4: ARCHIVE, R3-B1 closed; R4-N1 = #1039, R4-N2 by design

_2026-09-09T02:32Z · per R16, R17_

D106 round 4 (dev 76d3ef2e, targeted at R3-B1) returned ARCHIVE: the `Not inferred: no table or enum to declare in schema "X".` lines sort by code points in `import`, `pull` and the starter header for every flag order built (round 3's inputs verbatim, reverse code-point order, a capital beside lowercase, a repeated flag, an absent schema beside an empty one, NFC/NFD pairs under C/en_US/ko_KR); both refusal diagnostics name each schema once in code-point order (R3-N5 closed); every list on a two-schema catalog built so that flag, creation and physical order all disagree with code-point order sorts correctly, and `check`'s inventory agrees; the `band` and `loc` replays are byte-identical to round 3. Triage (412/D29): R4-N1 (a repeated `--schema` on a successful `pull` names the schema twice in the `pulled` line, the lock and the contract metadata) is #1039, reproduced there; R4-N2 (the starter's `foreignKeys` list sorts by the key's columns, its derived name, not by the `name:` it prints, unlike the sibling `indexes`/`checks` lists) is by design — locale-free and code-point ordered on its key — and recorded here as the one asymmetry a reader will notice. Four rounds in total for this change: 2 blocking findings in round 1, 2 in round 2, 1 in round 3, 0 in round 4; 32 non-blocking findings triaged into 13 issues, 6 by-design rulings and docs. Corpora: /private/tmp/d106-cf, d106-cf-r2, d106-cf-r3, d106-cf-r4.
