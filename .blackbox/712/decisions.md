...
<a id="r16"></a>
## R16 — nothing-to-infer means no table or enum to declare; a sequence-only or function-only schema earns it beside its own line, and no refusal silences the report

_lead · interpretation · basis R15, R14 · 2026-09-08T22:33Z · ratified: pending_

Correction of R15's boundary sentence. R15 reserved `nothing-to-infer` for "a schema holding no table, enum, sequence or function"; the piece measured that the catalog reading cannot attribute a function to a schema (the function line carries no schema), so that sentence could not be implemented as written. The definition, the text of `import-nothing-to-infer`/`pull-nothing-to-infer` and the delta sentence say what the reading decides on: the schema holds no table or enum to declare. A schema holding only a standalone sequence or only functions receives that refusal together with its own Not-inferred line naming what it does hold, so nothing is silent (the R2-N3 direction). The rest of R15 stands: a schema whose objects were all omitted for their names prints its report and is refused as `nothing-declarable`.
Both refusals print the loss report on stdout before exiting 1 with the diagnostic on stderr — a refusal never suppresses the report that precedes it — so requirement 2's "every command … SHALL print a loss report" holds on the refusal paths too, on `import` and on `pull` alike.

<a id="r17"></a>
## R17 — the two refusals split on what a schema lost, not on how its name looks: content-first classification, no way out that leads nowhere

_lead · interpretation · basis R15, R16 · 2026-09-08T22:33Z · ratified: pending_

R17 (interpretation, lead, 2026-09-09) — the two refusals split on what a named schema lost, not on how its name looks. `nothing-declarable` is the refusal when at least one named schema lost a table or enum to a name no declaration can carry — the object's own name or its schema's — and it names those schemas, because renaming brings something in. `nothing-to-infer` is the refusal otherwise: no named schema held a table or enum to declare, whatever its name (an empty schema, or one holding only a standalone sequence or a function), and such a schema's own `Not inferred:` line still prints. An empty schema whose own name cannot be carried is therefore `nothing-to-infer`, not `nothing-declarable`: the constructor reviewer followed the rename the other classification prescribes and arrived at a second refusal with nothing to merge — a way out that leads nowhere is a wrong classification, not a rough edge. Such a schema earns no `Omitted: schema` line either — that line's way out ("rename … then merge the declaration") could never be followed to its end for a schema holding nothing, and a line that names the way out names the whole of it — so it earns the `Not inferred: no table or enum to declare in schema "X".` line like any other schema with nothing to declare; the diagnosis that its name cannot be carried returns, on the `Omitted: schema` line, the moment a table or enum lands in it. A schema whose own name kept the reading out never earns lines for a sequence or function it held (they are filtered with it), so the delta says only that the report still names such a schema on its own `Not inferred:` line — inside the `nothing-to-infer` refusal an `Omitted: schema` branch is unreachable by construction (a name that cost a table or enum classifies as `nothing-declarable`), and the `Omitted`-versus-`Not inferred` split is stated once, in the reference's sentence on schema names ("wherever that name costs it something … where it costs nothing …"). A named schema is therefore in one of three states — its name can be carried; it cannot and a table or enum was lost to it; it cannot but nothing was lost — and the third is its own branch, never routed through `declareSchema`'s `invalid-sql-name` refusal. Closing the schema-level discriminator exposed a back door — `schemaHeldAnUncarriableName` matched the schema's own quoted name as object-level evidence and revived the old classification — removed in the same correction. The delta's requirement 1 says it in those words ("lost a table or enum to a name no declaration can carry — the object's own name or its schema's"). Basis: R15/R16 (the two codes exist to name what is missing and its way out), the delta's own "held something whose name no declaration can carry", constructor review B2 (piece #1051). Ratification: queued for the owner.
