...
Review round 2 (LL2/MM2): the authoritative list is the union of the index's key columns and the columns `pg_depend` records for its expression and predicate -- measured: an index a constraint backs records only index -> constraint in `pg_depend` (t2_pkey, t2_val_key both refclassid pg_constraint), so its key columns never appear there. The live witness caught this: a unique index backing a constraint on an enum-omitted column replayed as `column "value" does not exist`.
