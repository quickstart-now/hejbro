...
N3 was first routed to #946 and then pulled into this PR on the planner's fail-closed argument: a contract-breaking driver silently handing the caller a context statement's rows is a wrong answer rather than an error, which this repository's design does not admit. The query layer now checks that a driver returned one row list per member and refuses with `batch-result-count-mismatch` otherwise; #946 is closed by this PR. `lastBatchRows`'s former zero-result invariant converges into that diagnostic -- zero is one case of a count mismatch -- while `result-rows.ts`'s own zero-length invariant (486/R6, the multi-command fold) is a different place and stays as it is.
