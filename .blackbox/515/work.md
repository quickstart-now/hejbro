...
<a id="w3"></a>
## W3 — the and bullet's path claim corrected after re-review

_2026-09-05T20:39Z_

Re-review of the corrected sentences. The AND bullet's first path claim was wrong: a `with` body and a set operation's branches are `SelectNode | SetOpNode` positions the lifter reaches through its own query walkers, and `selectChildExprs` applies at each `SelectNode` leaf, not at those positions directly -- the bullet now says so. The changeset drops the unpublished example kind from the release note, naming only `supabase-storage-bucket`. No code or test changed in this round; gates rerun: openspec validate --strict, check:modified-titles, check:pr-changeset, blackbox check.
