...
<a id="w5"></a>
## W5 — the last two precision fixes: a one-way inclusion and a type-resolution limit

_2026-09-05T22:39Z · per R13_

The second re-review passed with zero blockers and two non-blocking notes, both adopted as the last precision fixes. N6: the requirement's opening said "the pairs the server unifies are the same-family ones" -- a two-way equality that also claims every same-family pair unifies, which the very next paragraph's own list of same-family refusals (an array against an array of a mismatched element type, two distinct enums, `json` against `jsonb`, a time-of-day type against a date/timestamp type, `macaddr` against `inet`/`cidr`) refutes; the sentence now states only the direction the rule relies on ("every pair the server unifies is a same-family one"). N7: scenario 2a's THEN said the compiled statement "is the one Postgres accepts by typing the untyped side against the other branch" without qualification; the reviewer's own value-level reproduction showed an untyped literal whose text does not parse as the resolved type is refused there regardless (`'x'` against `integer`, `22P02`; `'x'` against `timestamptz`, `22007`), so the THEN now names type resolution as the boundary of what this scenario claims, leaving the literal's own text a value-level question the server answers at execution. The same two patterns were checked against `query-layer.md` and design.md's Q1, which carried the identical unqualified mechanism sentence, and corrected the same way.
