...
<a id="w5"></a>
## W5 — NB-3: the banner names the primary key on every adoption that creates one

_2026-09-08T14:35Z · per R9_

NB-3 (lead ruling, final review round): the migration banner's primary-key note condition widened.

W3's F3 bullet describes the note as it first shipped: fired only when an adoption's declared primary key is its *sole* creation, so `diffs` is otherwise empty (`isEmptyTableFieldDiffs(diffs) && isAdoption && next` declares one). The lead reviewed that scoping and ruled it too narrow — across the review's own corpus, 11 adoption cells create a primary key and only 3 of them named it in the banner, so the rule a user could observe from the banner alone was unpredictable — and widened the condition to `isAdoption` alone: the primary key is now named on every adoption that creates one, whatever else the same adoption also creates.

`table-kind.ts`'s `adoptionCreatesPrimaryKey`/`primaryKeyOnlyAdoptionNote` were replaced by `adoptionPrimaryKeyName(isAdoption, next): string | null` (the one guard) and `adoptionPrimaryKeyNote(primaryKeyName)`; `isEmptyTableFieldDiffs` stays only in the unrelated decision of whether the table's own alter fires at all.

Explicitly not suppressed: an adoption cell whose existing side never listed the primary key already produces a `column "…" changed` note (the flag lives on the column, D68) — that adoption's banner now reads `[column "id" changed, primary key "<t>_pkey" added]`, naming the same fact at two layers on purpose, since the banner states what the file does and the file always carries the `add constraint … primary key` statement whenever the note fires.

The one fence kept is `isAdoption`, measured by mutation: deleting it from `adoptionPrimaryKeyName` reddens 9 `generate.test.ts` cells — seven of them managed→managed or new-table (no-op ×3, serial ×2, managed unchanged ×2), but two are not: `"an adopted table creates its declared foreign key, the target untouched (671/task 1.1, table A: foreign key x adoption)"` and `"a handover stays fully silent for a table that declares a foreign key (671/task 1.1, table A: foreign key x handover)"` — an adoption cell and a handover cell. What the mutation actually shows is narrower than "guards non-adoption paths": it guards every cell whose own expectation would otherwise gain or lose a spurious primary-key note, adoption and handover included, not only managed→managed and new-table. Across the full suite the same mutation reddens 58 tests in 9 core files, including 10 goldens; confirmed and reverted, not committed.

Updated: `uo20`, `uo23` (single/composite, no other child, existing side lacked the key) and `b1b`, `b1d` (single/composite, with another child, existing side already had the key) all gained the note; `b1a`/`b1c`/`uo6` (already fixed under F3) are unchanged. `examples/*` and `cli-smoke` goldens read clean under the new condition — none exercise an adopted primary key.
