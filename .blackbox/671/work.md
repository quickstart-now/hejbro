...
<a id="w4"></a>
## W4 — W3 corrections: the f1-f6 drop-order claim, and the banner's primary-key note condition widened (NB-3)

_2026-09-08T13:43Z_

Two final-review corrections to W3 (append-only: W3's text above is left as it was written, not edited).
...
**(b) NB-3 — the banner's primary-key note condition widened (lead ruling).** W3's F3 bullet above describes the note as it first shipped: fired only when an adoption's declared primary key is its *sole* creation, so `diffs` is otherwise empty (`isEmptyTableFieldDiffs(diffs) && isAdoption && next` declares one). The lead reviewed that scoping and ruled it too narrow — across the review's own corpus, 11 adoption cells create a primary key and only 3 of them named it in the banner, so the rule a user could observe from the banner alone was unpredictable — and widened the condition to `isAdoption` alone: the primary key is now named on every adoption that creates one, whatever else the same adoption also creates. `table-kind.ts`'s `adoptionCreatesPrimaryKey`/`primaryKeyOnlyAdoptionNote` were replaced by `adoptionPrimaryKeyName(isAdoption, next): string | null` (the one guard) and `adoptionPrimaryKeyNote(primaryKeyName)`; `isEmptyTableFieldDiffs` stays only in the unrelated decision of whether the table's own alter fires at all. Explicitly not suppressed: an adoption cell whose existing side never listed the primary key already produces a `column "…" changed` note (the flag lives on the column, D68) — that adoption's banner now reads `[column "id" changed, primary key "<t>_pkey" added]`, naming the same fact at two layers on purpose, since the banner states what the file does and the file always carries the `add constraint … primary key` statement whenever the note fires. The one fence kept is `isAdoption`: measured by mutation — deleting it from `adoptionPrimaryKeyName` reddens 9 `generate.test.ts` cells, all managed→managed or new-table, confirmed and reverted, not committed. Updated: `uo20`, `uo23` (single/composite, no other child, existing side lacked the key) and `b1b`, `b1d` (single/composite, with another child, existing side already had the key) all gained the note; `b1a`/`b1c`/`uo6` (already fixed under F3) are unchanged. `examples/*` and `cli-smoke` goldens read clean under the new condition — none exercise an adopted primary key.
