...
**671/R10 (B2) branch confirmation basis**, measured by the researcher on dev `dbd74fbc`/`408e9996` (`/private/tmp/har1-research/logs/`, `REPRO-mid-chain-baseline.md` plus the labeled run logs) — pre-671/R9 throughout except where noted inline as independently reconfirmed against this PR's own 2.1+2.2 build:
...
(C) With no mid-chain registration path, both ways in branch (b) were measured on the same `p3b`-shaped state (managed → handed over → re-adopted):
  ...
  - Drop: `e1`-`e3`, `d0`-`d4` and `f1`-`f6` were all measured against the same pre-671/R9 dev SHA this basis names above (`dbd74fbc`/`408e9996`), before the fix existed. `e1`-`e3`: dropping the index and the check alone was sufficient, since no primary key was ever created to collide; `d3` additionally measured that dropping the sequence too leaves `check-object-differs: app.orders.id` (N1, the `nextval` default never re-attaches) with no `generate` able to self-heal. `f1`-`f6` explored what an eventual PK-creating fix would still need dropped by hand-appending an `alter table … add constraint "orders_pkey" primary key ("id");` statement to the *tail* of the already-written pre-fix migration file (`p3bc/migrations/20260907144403_add_orders_id_seq.sql` — its own banner carries no primary-key note, matching the pre-fix CLI's real output) — not a native emission of any build, and its tail position does not reflect 671/R9's own statement order once implemented: against that simulated file, `f1` (nothing dropped) fails `42P07` first because the appended statement sorts after the index; `f2` (index and check dropped) then fails `42P16: multiple primary keys for table "orders" are not allowed` on the appended statement; `f3` (the primary key dropped as well) applies cleanly, `f4`-`f6` confirm `status`/`check`/`verify` all clean afterward. 671/R9's real, unedited build sorts the opposite way — the primary key's own `add constraint` ahead of the index and the check (`table-kind-emit.ts`'s `emitAlter`) — independently confirmed by this PR's own live-witness test (`adoption-handover.integration.test.ts`'s B2 drop path): a single "nothing dropped" attempt against the real build fails `42P16` immediately and never reaches `42P07` at all. Two further risks apply only to the primary-key member of the drop set (not index/check/FK): dropping a primary key another table's foreign key still references is refused by Postgres (`cannot drop constraint … because other objects depend on it`), and a row inserted between the drop and the apply can make the primary key's re-creation fail on a duplicate.
