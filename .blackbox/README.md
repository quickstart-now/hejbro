...
| ↳ [#486](486/) | the driver contract has no vocabulary for non-interactive statement batches | closed | #955 #962 | 0 | 14 (5 pending) |
...
| ↳ [#503](503/) | reject cross-family set-operation branches at build time | closed | #992 #999 #1062 | 0 | 14 (5 pending) |
...
| ↳ [#668](668/) | generate: warn when adopting an existing table will create sequences, RLS or policies that may already exist | closed | #1019 #1043 #1046 | 0 | 0 (0 pending) |
| ↳ [#671](671/) | existing: adoption never creates the adopted table's declared indexes, checks, FKs or primary key | closed | #1019 #1043 #1046 | 0 | 12 (5 pending) |
...
| ↳ [#678](678/) | infer: role names from policies, not only from grants | closed | #1026 #1049 #1060 #1066 | 0 | 0 (0 pending) |
...
| ↳ [#694](694/) | handover then adoption fails to apply: sequence already exists | closed | #1019 #1043 #1046 | 0 | 0 (0 pending) |
...
| ↳ [#712](712/) | decide whether an enum type's catalog name is held to D36 by the catalog reading | closed | #1026 #1049 #1060 #1066 | 0 | 17 (4 pending) |
...
| ↳ [#738](738/) | execute() of a core-built set operation cannot widen a column to the union of both branches | open | #1062 | 0 | 5 (4 pending) |
...
| [#444](444/) | select-node traversal misses its new clause fields at every site | closed | #451 #988 | 0 | 0 (0 pending) |
...
| [#815](815/) | 0.2.x: post-release commitments (2) | open | #867 #868 #871 #878 #968 #976 #979 #980 #988 #991 #992 #999 #1005 #1010 #1011 #1014 #1019 #1026 #1027 #1028 #1041 #1043 #1046 #1049 #1060 #1062 #1066 #893 (via #814) #910 (via #816) #924 (via #816) #840 (via #819) #876 (via #820) #835 (via #823) #877 (via #823) #833 (via #832) #838 (via #836) #851 (via #841) #855 (via #841) #849 (via #845) #850 (via #845) #847 (via #846) #858 (via #856) #861 (via #856) #955 (via #891) #962 (via #891) #900 (via #899) #963 (via #931) #956 (via #932) | 0 | 1 (1 pending) |
...
| ↳ [#865](865/) | ledger: forced row-level security hides the ledger's rows and migrate re-applies the chain instead of refusing | closed | #871 #877 #1005 #1010 | 0 | 1 (1 pending) |
...
| ↳ [#872](872/) | import: a non-default primary-key constraint name is dropped with no loss-report line | closed | #878 #1026 #1049 #1060 #1066 | 0 | 1 (1 pending) |
| ↳ [#873](873/) | import: a foreign key referencing an omitted column makes the starter declaration fail to load | closed | #878 #1026 #1049 #1060 #1066 | 0 | 0 (0 pending) |
| ↳ [#874](874/) | loss report: sortedBy uses localeCompare, so import/pull output order depends on the locale | closed | #878 #1026 #1049 #1060 #1066 | 0 | 0 (0 pending) |
...
| ↳ [#891](891/) | query: one exported statement-name helper for every driver that prepares | closed | #955 #962 | 0 | 0 (0 pending) |
| ↳ [#892](892/) | pg driver: a multi-command sql text resolves to undefined rows | closed | #955 #962 | 0 | 0 (0 pending) |
...
| ↳ [#942](942/) | query: db.with reads every CTE key nullable (untracked boundary), so the recursive-nullability widening is not observable on the chain surface | open | #956 #1062 | 0 | 0 (0 pending) |
...
| ↳ [#944](944/) | query: a set operation's result row type uses the left branch's projection only, so a nullable right branch reads non-null | closed | #1062 | 0 | 0 (0 pending) |
...
| [#978](978/) | core/cli: banner hash lines keep a trailing CR on a CRLF checkout, so verify and history compare sha256:…\r against … | open | #1005 #1010 | 0 | 0 (0 pending) |
| [#981](981/) | cli integration suites drifted from examples/postgres after #900: check-live 6.3 and declare-emit-roundtrip 2.2 fail on dev | open | #1005 #1049 | 0 | 0 (0 pending) |
...
| [#995](995/) | 0.2.x: post-release commitments (3) | open | #999 #1005 #1010 #1011 #1014 #1019 #1026 #1027 #1028 #1041 #1043 #1046 #1049 #1060 #1062 #1066 | 0 | 0 (0 pending) |
...
| ↳ [#1000](1000/) | query: the chain related() returns has no offset() stage while the plain select chain does | open | #1011 #1014 | 0 | 0 (0 pending) |
| ↳ [#1001](1001/) | generate: adoption's banner notes describe column diffs that emit no statements | open | #1019 #1043 #1046 | 0 | 0 (0 pending) |
...
| ↳ [#1004](1004/) | vendor: two carried tables with the same name in different schemas collide in the contract's Tables keys | closed | #1011 #1014 #1028 #1041 | 0 | 1 (1 pending) |
...
| ↳ [#1009](1009/) | adoption: re-adopting a table a handover left in the database re-creates its policies and constraints (42710 / 42P16) | open | #1019 #1043 #1046 | 0 | 0 (0 pending) |
| ↳ [#1012](1012/) | query: related() accepts a string spec at the type level and fails at run time with unknown-relation "0" | open | #1014 | 0 | 0 (0 pending) |
| ↳ [#1013](1013/) | vendor: a relative path given to hejbro link is resolved against the fetch's temp dir, so vendor fails with vendor-remote-unreachable | open | #1014 | 0 | 0 (0 pending) |
| ↳ [#1015](1015/) | adoption: a column-level unique() is neither created nor named by adoption-creates | open | #1019 #1043 #1046 | 0 | 0 (0 pending) |
| ↳ [#1016](1016/) | import: a cross-schema handle's columns are emitted with arbitrary types (text() for an int column) | open | #1026 #1049 #1060 | 0 | 0 (0 pending) |
| ↳ [#1017](1017/) | baseline: a primary key whose derived name is already taken by an index on the same table emits the name twice | open | #1026 | 0 | 0 (0 pending) |
| ↳ [#1018](1018/) | loss report ordering: no server-collation axis and no unit test for compareCodeUnits | open | #1026 | 0 | 0 (0 pending) |
| ↳ [#1020](1020/) | import: the primary-key name collision check does not see composite types or foreign tables holding the derived name | open | #1026 | 0 | 0 (0 pending) |
| ↳ [#1021](1021/) | import: a primary key over an omitted column is silently weakened (composite) or dropped without a line | closed | #1026 | 0 | 0 (0 pending) |
| ↳ [#1022](1022/) | import: a generated stored column loses its expression, so the starter fails the next check | closed | #1026 #1049 | 0 | 0 (0 pending) |
| ↳ [#1023](1023/) | import: an index key's COLLATE clause is dropped silently | open | #1026 | 0 | 0 (0 pending) |
| ↳ [#1025](1025/) | loss report: a key that lost several members names one member and one way out at a time | closed | #1026 | 0 | 0 (0 pending) |
| ↳ [#1029](1029/) | generate: adopting a table in a schema the snapshot never recorded emits create schema, which the notice does not name and which fails first | open | #1046 | 0 | 0 (0 pending) |
| ↳ [#1030](1030/) | generate: a column drop+add in the same edit as an adoption is not refused as an ambiguous rename | open | #1046 | 0 | 0 (0 pending) |
| ↳ [#1031](1031/) | generate: warning[adoption-creates] carries no at <file>:<line> location, unlike every other warning | open | #1046 | 0 | 0 (0 pending) |
| ↳ [#1032](1032/) | import/pull: column-level and sequence-level grants contribute no role name to the reading | open | #1049 | 0 | 0 (0 pending) |
| ↳ [#1033](1033/) | import: an index's INCLUDE columns are dropped silently, and the omitted-index line calls an INCLUDE list an expression | open | #1049 #1060 | 0 | 0 (0 pending) |
| ↳ [#1034](1034/) | import: partitioning, inheritance, UNLOGGED, comments and RLS enablement are dropped without a Not inferred line | open | #1049 #1060 | 0 | 0 (0 pending) |
| ↳ [#1037](1037/) | migrate: an adoption migration whose objects the database already holds has no official path to be registered instead of run | open | #1043 #1046 #1060 | 0 | 0 (0 pending) |
| ↳ [#1038](1038/) | vendor/pull: the table-name-collision diagnostic heads with one table (vendor) or "Tables" (pull) and lists every collision on one unwrapped line | open | #1041 | 0 | 0 (0 pending) |
| ↳ [#1039](1039/) | pull: --schema repeated with the same name is accepted and recorded twice in the lock and the contract metadata | open | #1041 #1060 #1066 | 0 | 0 (0 pending) |
| ↳ [#1040](1040/) | vendor: --ref <commit sha> does not resolve on a local-path source, while a tag on the same commit does | open | #1041 | 0 | 0 (0 pending) |
| ↳ [#1042](1042/) | check: a primary key whose derived name another constraint already holds is reported as missing, not as a name collision | open | #1043 #1046 | 0 | 0 (0 pending) |
| ↳ [#1044](1044/) | generate: two runs in the same second write the same timestamp version, so verify refuses the chain (cli-smoke e2e flake on fast runners) | open | #1046 | 0 | 0 (0 pending) |
| ↳ [#1045](1045/) | adoption: dropping a held primary key with cascade removes a managed foreign key that generate never re-creates | open | #1046 | 0 | 0 (0 pending) |
| ↳ [#1047](1047/) | import/pull: the 'nothing to infer in schema' line prints after the Omitted band, against the stated band order | closed | #1049 | 0 | 0 (0 pending) |
| ↳ [#1048](1048/) | import/pull: a column unreadable for both its name and its type gets the rename remedy, which cannot resolve it | open | #1049 #1060 | 0 | 0 (0 pending) |
| ↳ [#1052](1052/) | import: a destination refusal after a completed catalog reading throws without printing the loss report | open | #1060 | 0 | 0 (0 pending) |
| ↳ [#1053](1053/) | query: a CTE reference drops the nullability its entry acquired by left-joining | open | #1062 | 0 | 0 (0 pending) |
| ↳ [#1054](1054/) | query: a set operation with same-family width divergence converts rows with the left branch's converters, so a promoted column arrives raw | open | #1062 | 0 | 0 (0 pending) |
| ↳ [#1055](1055/) | query: handle.execute of a with-stage reads back as untyped driver rows | open | #1062 | 0 | 0 (0 pending) |
...
| ↳ [#1059](1059/) | test(core): promote the reviewer's CTE-surface set-op fold cells to the shipped suite | open | #1062 | 0 | 0 (0 pending) |
...
| ↳ [#1064](1064/) | import: a foreign key onto a column whose only uniqueness was omitted for its name is kept, and the baseline cannot apply | open | #1066 | 0 | 0 (0 pending) |
| ↳ [#1065](1065/) | docs: a table's owner lands in Guessed role names and the vendored roles with no grant present | open | #1066 | 0 | 0 (0 pending) |
