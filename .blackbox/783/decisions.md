...
<a id="r6"></a>
## R6 — D106 round 1 disposition: partition and inheritance child refused (NB1), letter-free fallback (NB2), #836 #837 filed, archived

_lead · interpretation · basis D1, R2, R5 · 2026-09-04T15:34Z · ratified: pending_

D106 round 1 (context-free, opus, dev f548272c, live PG 17.11): BLOCKING 0 / NON-BLOCKING 3 / OK 6. Disposition under the delegation: NB1 (a leaf partition and an inheritance child are `relkind = 'r'` and were judged the ledger — the reviewer wrote a row into and then deleted a row from a stranger's inherited table) is repaired at the archive as group 3: the probe reads `relispartition` and `pg_inherits`, both shapes are refused and named in words, the requirement says what "ordinary" meant — a logged table hejbro could have created. This is the identity principle this change rests on (hejbro touches only what it created), not a widening; inside hejbro's purpose (D13 on dev). NB2 (the fallback word carried the catalog letter the requirement forbids) repaired in the same task. NB3 (a permission error on a real ledger reaches `status` raw) → #836; the out-of-delta `raise --file` absolute-path raw ENOENT → #837. Archived.
