// #154/#278: the CRAP (complexity^2 * (1 - coverage)^3 + complexity) walker
// and its analysis of every named function in @hejbro/core and
// @hejbro/supabase -- factored out of check-crap.mjs (phase9-crap-readme,
// #278) so `scripts/check-crap.mjs` (the CI gate) and
// `scripts/update-crap-readme.mjs` (the README block/badge, #278) compute
// off the exact same `computeCrapReport()` results and can never report
// different numbers. This file is the computation; check-crap.mjs and
// update-crap-readme.mjs are both thin CLI wrappers around it.
//
// phase8-crap-tooling through phase8-crap-coverage brought the violation
// count to 0 at threshold 10, and phase8-crap-gate turned
// EXIT_NONZERO_ON_VIOLATION on; phase8-crap-ratchet-5 ratcheted the
...
// (see the missing-coverage check in computeCrapReport below) rather than
...
// Runs the full scan and returns `{ results, violations }` -- `results` is
// every scanned function across TARGET_PACKAGES, `violations` is the
// subset over CRAP_THRESHOLD, sorted worst-first. Exits the process
// directly (2/3) on the two "the input data itself is unusable" cases,
// same as before this file was split out of check-crap.mjs (#278) --
// both callers (the CI gate and the README refresher) want that failure
// mode identically, so there is nothing a caller-specific error path
// would buy either one.
export const computeCrapReport = () => {
	// Every scanned file MUST have its own entry in coverage-final.json.
	// The `!coverage` branch below already fails loudly (exit 2) when the
	// coverage report is missing entirely; this array is the same
	// treatment for the narrower case of one scanned file's key being
	// missing from an otherwise-present report -- a renamed/moved file, a
	// narrowed `coverage.include`, a cross-worktree cache hit predating
	// `cache: false`, or any other path mismatch. Collected across every
	// package before failing (rather than exiting on the first miss) so
	// one run reports the complete list, not just the first offender.
