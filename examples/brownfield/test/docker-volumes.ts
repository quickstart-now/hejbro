...
/**
 * Inlined from `packages/cli/test/docker-volumes.ts` (planner instruction,
 * #714 bc-1): this package doesn't reach into `packages/cli/test`, so its
 * own `#709` cleanup logic is duplicated here rather than imported --
 * the same choice `examples/postgres/test/integration.test.ts`'s own
 * `removeContainer` already made ("Not shared with the sibling copies in
 * `packages/cli/test/docker-volumes.ts`/`packages/pg/test/
 ...
 * removal, since `docker inspect` on a container `rm -f` already deleted
 * answers nothing. A bind mount's own `.Name` is empty (it has no volume
 * identity of its own), so only real named/anonymous volumes ever appear
 * here.
 */
 ...
 * volume -- every integration witness's own `afterAll` did this, and the
 * accumulation ate the shared Docker data disk (round 4, D106). `-v` on
 * `rm` frees the volume too; this checks that it actually happened,
 * since the flag's own success is silent -- the volume names this
 * container's own mounts carried, read before removal, must all be gone
 * from `docker volume ls` right after. Naming both the leftover volumes
 * and the container on failure is half this check's value.
 *
 ...
 * that one failure into `null`, but this function does not swallow it a
 * second time: `rm -f -v` still runs either way, and a `null` read is
 * reported afterward (not silently passed) as its own failure, since the
 * volume attribution this function exists to prove was never actually
 * checked.
 */
