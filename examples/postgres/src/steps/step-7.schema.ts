 ...
 * (`step-1` … `step-7`) evolve it; this is step 7 — `task_labels`, a
 * brand-new table, is added to a schema that already carries a standing
 * one-shot `all-tables-privileges` grant (`appAuditorSelectGrant`,
 * standing since step 1). Postgres's own `grant ... on all tables in
 * schema ...` only ever covers the tables that existed *when it ran*, so
 * without #121's fix `app_auditor` would end up silently unable to
 * select from this table — a defect a golden test can't see (it never
 * runs real SQL) but the local round-trip does: a chain database (this
 * committed migration applied after the earlier ones) vs. a fresh
 * database (one migration from empty straight to this state) disagree
 * on exactly `task_labels`'s grants before the fix, and agree after it
 * (D48/D49).
 */
