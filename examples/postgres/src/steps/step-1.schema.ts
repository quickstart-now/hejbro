 ...
 * (`step-1` … `step-7`) evolve it; this is step 1 — see step-2/3/4 for
 * what changes.
 */
...
export const projects = table(
	...
	(t) => ({
		foreignKeys: [
			// Step 1: default referential actions (no explicit onDelete/onUpdate) —
...
export const appWriterDefaultAllGrant = grant(app)
	...
	.to(appWriterRole);
// #121: standing from this step on, like the reader/writer grants above —
// but deliberately given *no* matching defaultPrivileges counterpart, so a
