...
const buildRegistry = () => {
	...
	registerPresets(registry, [supabasePreset]);
	return registry;
};
// Step 4 moves storage_path across tables — the same --confirm-drop the CLI asked for.
		...
		const outcome = steps.reduce(
			(state, declarations, i) => {
				const result = generateMigration({
					...
					validators: supabasePreset.validators,
