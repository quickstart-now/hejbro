 ...
 * (`step-1` … `step-4`) evolve it; this is step 2 — adds
 * `attachments.content_type` and widens the bucket's allowed mime types.
 */
...
export const attachments = table(
	...
	"attachments",
	{
		...
		// Nullable — a CHECK still applies once set, but no default is needed
		// (avoids the not-null-without-default warning, #27).
		contentType: text(),
	},
	(t) => ({
		foreignKeys: [
...
// Step 2: widened allow-list — exercises the bucket's row-data alter path.
