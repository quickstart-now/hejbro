...
// #674: the platform-owned table the schema reads and references, declared
// for its shape (`existingTable()`, add-unmanaged-objects) so the snapshot
// records it as existing and `check` names it in its coverage boundary.
// It must be a *named export*: the loader collects a module's exported
// declarations one by one and never reads the `declarations` array below.
export { authUsers };
