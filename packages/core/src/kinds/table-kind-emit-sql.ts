...
/** `IdentitySnapshot.kind`'s own SQL phrase. */
...
/**
 * Renders an identity column's clause: its kind's phrase, plus
 * `(start with N)` when declared. Only `startWith` renders today — the
 * other five `IdentityOptions` keys have no confirmed SQL token yet
 * (D100); a declaration setting one is not yet representable in emitted
 * SQL. `[]` for a non-identity column.
 */
