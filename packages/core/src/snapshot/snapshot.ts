 ...
 * a format at or above it names {@link olderReleasedFormatMessage}'s
 * `hejbro upgrade` path instead (#413, 1.2).
 */
const HEJBRO_UPGRADABLE_SNAPSHOT_FLOOR = 5;

/**
 * `snapshot version <v> is older than this build supports …` for a format
 * at or above the release floor (#413, 1.2) — unlike
 * {@link olderVersionMessage}'s pin-or-reset guidance for a format no
 * release ever wrote, a real forward path exists. Ends with the upgrade
 * command and never mentions pinning or resetting.
 */
const olderReleasedFormatMessage = (version: number): string =>
	`snapshot version ${version} is older than this build supports (expects ${HEJBRO_SNAPSHOT_VERSION}) — this is a format from this build's own snapshot history, not one from before any release existed. Next: run \`hejbro upgrade\`.`;

/**
 * {@link validatePresentFormatVersion}'s older-format branch (#413, 1.2):
 * below the release floor keeps {@link olderVersionMessage}'s pin-or-reset
 * guidance unchanged; at or above it (a format this build's own history
 * wrote, one {@link upgradeSnapshot} can re-encode) names `hejbro upgrade`
 * instead.
 */
const olderFormatMessage = (version: number): string => {
	if (version < HEJBRO_UPGRADABLE_SNAPSHOT_FLOOR) {
		return olderVersionMessage(version);
	}
	return olderReleasedFormatMessage(version);
};
...
const validatePresentFormatVersion = (formatVersion: unknown): void => {
	...
	if (formatVersion < HEJBRO_SNAPSHOT_VERSION) {
		throwHejbroError(
			"unsupported-snapshot-version",
			olderFormatMessage(formatVersion),
		);
		return;
	}
	throwHejbroError(
