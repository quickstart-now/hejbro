...
/** The measured no-primary-key shape (#573), declared only for its typed `db()` handle; created with raw DDL inside its own witness. */
...
describe("nileDriver + a real db() handle against Nile's official testing container (task 5.1-5.6, #567)", () => {
	...
	// witness D: the four measured-only refusals the preset's validators
	// rest on, re-measured against this digest (D106 F1 on #573: a spec
	// sentence that says "measured" needs the measurement in the file the
	// header promises to re-run). Raw DDL, like the fixtures above -- the
	// claim is what the *server* answers to the shape, not what
	// generateMigration renders (the unit suite pins that half).
	it.each([
		[
			"a serial column",
		...
		[
			...
			'create table "app"."m_pk" (tenant_id uuid not null, id uuid primary key)',
			/primary key of tenant-aware table must have the "tenant_id" column/,
		],
		[
			"a generated always as identity column",
			'create table "app"."m_id_always" (tenant_id uuid not null, seq integer generated always as identity)',
			/IDENTITY columns are not supported for tenant-aware table/,
		],
		[
			"a generated by default as identity column",
			'create table "app"."m_id_default" (tenant_id uuid not null, seq integer generated by default as identity)',
			/IDENTITY columns are not supported for tenant-aware table/,
		],
	])(
		"witness D: the server refuses %s in a tenant-aware table (measured-only refusal, re-measured on this digest)",
		async (_shape, sql, expected) => {
			await expect(
				(base.current as ReturnType<typeof pgDriver>).execute({
					sql,
					params: [],
					kind: "sql",
				}),
	...
	it("witness D: a tenant-aware table with no primary key at all is accepted and takes a row under a tenant context (the shape the preset leaves alone)", async () => {
		await (base.current as ReturnType<typeof pgDriver>).execute({
			sql: 'create table "app"."notes" (tenant_id uuid not null, name text)',
			params: [],
			kind: "sql",
		});
		const handle = db(
		...
		await handle
			.as(asTenant(TENANT_A))
			.insert(notes)
			.values([{ tenantId: TENANT_A, name: "keyless" }]);

		const rows = await handle.as(asTenant(TENANT_A)).select(notes);
		expect(rows.map((row) => row.name)).toEqual(["keyless"]);
	});
});
