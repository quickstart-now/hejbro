...
describe("nilePreset is a registrable bundle from the public entry (task 1.2/4.1-4.4, #563/#566, plus a fifth validator added after G5's live-witness measurement, #567)", () => {
	it("carries the shape a Preset must -- name, kinds (still empty, task 1.2's own scope), validators (populated additively by group 4, and again after G5)", () => {
		expect(nilePreset.name).toBe("nile");
		expect(nilePreset.kinds).toEqual([]);
		expect(nilePreset.validators).toHaveLength(5);
	});
	...
	it("presetValidators flattens it to the five platform-refusal validators the preset carries", () => {
