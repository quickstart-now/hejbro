...
- 36dd132: The ledger records a body checksum, migrate refuses an applied migration whose body changed, status reports it, and a ledger whose rows are filtered by row-level security is refused before it is read.
...
- 3b7e0d6: The vendored contract names each table's relations and the name-keyed client's select chain offers `.related()`.
...
- ea925bf: Adopting an `existingTable()` back into a managed `table()` now also
...
- 26b623e: Adoption now creates a table's declared primary key even when the
...
- 779e7bb: `hejbro import`/`pull` now read a stored generated column as a
  ...
  line instead.
- 76d3ef2: `import` and `pull` print their `Not inferred: no table or enum to declare in schema "X".` lines sorted by code units, each schema once, whatever the `--schema` order — the same order every other list in the loss report already uses — and the `*-nothing-to-infer` / `*-nothing-declarable` refusals list schemas the same way.
- becacf0: `hejbro import`/`pull`'s brownfield-adoption reference now names three
...
- 9ebeef8: A schema whose only objects were omitted for their own name (a table
...
- a5accf5: `vendor` and `pull` refuse to emit a contract that carries two tables of one SQL name in different schemas (`vendor-table-name-collision`, `pull-table-name-collision`), naming every colliding name with its qualified tables, instead of writing a `contract.ts` that does not compile (#1004).
- 35af6b1: A set operation returned as the body of `db.with(...)` now reads back as the union of its branches (a column nullable only in the right branch reads nullable, a column declared at two widths reads their union), the same fold `execute()` and the chain apply; a row read over a CTE reference no longer carries the reference's internal symbol key.
...
- Updated dependencies [ea925bf]
- Updated dependencies [761567b]
- Updated dependencies [99b9554]
- Updated dependencies [a1fb7e7]
