 ...
 * check): whether the reading saw something in this schema that it
 * could not carry for its own name, schema-qualified.
 ...
 * never one already folded with this function's own output.
 */
 ...
 * `schemasHoldingAnUncarriableName` (712/R15): every requested schema
 * the reading saw something in but could not carry for its own name --
 * a schema omitted whole for its own name (`result.omittedSchemaNames`)
 * or one holding an object `schemaHeldAnUncarriableName` finds. The
 * classification counts what the reading saw, not what it kept.
 */
