...
/** Catches a promise expected to reject and returns what it threw -- `fail()` on the (unexpected) resolved path, since a caller that reaches here always wants the tag to build a follow-up assertion from, never the value. */
const caught = async (promise: Promise<unknown>): Promise<unknown> => {
	...
	throw new Error("caught: the promise resolved instead of rejecting");
};

describe("a ledger read the server refuses is a coded diagnostic / 1.2 (harden-ledger-diagnostics)", () => {
	const Command = "hejbro status";

	it.each<[string, unknown]>([
		["readLedger, 42501 select withheld", permissionDeniedTable()],
		["readLedger, 42501 schema usage withheld", permissionDeniedSchema()],
		["readLedger, a bare error with no code", bareError()],
	])("%s -> apply-ledger-unreadable naming the role", async (_label, error) => {
		const { session } = makeScriptedSession([
			{ throws: error },
			{ rows: [{ currentUser: "ld_noselect" }] },
		]);
		const tag = await caught(readLedger(session));

		await expect(
			throwLedgerReadFailure(session, tag, Command),
		).rejects.toSatisfy((thrown: unknown) => {
			...
			return (
				...
				thrown.message.includes(Command) &&
				(thrown as unknown as { readonly cause: unknown }).cause === error
			);
		});
	});

	it("isMigrationRecorded, 42501 -> apply-ledger-unreadable, same shape as readLedger's own", async () => {
		const error = permissionDeniedTable();
		const { session } = makeScriptedSession([
			{ throws: error },
			{ rows: [{ currentUser: "ld_noselect" }] },
		]);
		const tag = await caught(isMigrationRecorded(session, "0001_init.sql"));

		await expect(
			throwLedgerReadFailure(session, tag, "hejbro migrate"),
		).rejects.toSatisfy((thrown: unknown) => {
			if (!(thrown instanceof HejbroError)) {
				return false;
			}
	...
	it("omits the role clause, never fails twice, when select current_user itself fails", async () => {
		const error = permissionDeniedTable();
		const roleLookupError = new Error("connection reset by peer");
		const { session } = makeScriptedSession([
			{ throws: error },
			{ throws: roleLookupError },
		]);
		const tag = await caught(readLedger(session));
		...
		await expect(
			...
			return (
				...
				// appears -- only the *naming* clause ("could not be read as
				// the role ...") is conditional on D2's lookup, so this checks
				// that specific clause, not the bare substring "as the role".
...
describe("a ledger write the database refuses names the ledger and what was being written / 1.3 (harden-ledger-diagnostics)", () => {
	const Command = "hejbro migrate";

	it("bootstrapLedger, 42501 -> apply-ledger-unwritable naming the bootstrap, no migration mentioned", async () => {
		const error = permissionDeniedDatabase();
		const { session } = makeScriptedSession([
			{ throws: error },
			{ rows: [{ currentUser: "ld_nocreatedb" }] },
		]);
		const tag = await caught(bootstrapLedger(session));

		await expect(
			throwLedgerWriteFailure(session, tag, Command),
		).rejects.toSatisfy((thrown: unknown) => {
			if (!(thrown instanceof HejbroError)) {
				return false;
			}
	...
	it.each<[string, unknown]>([
		...
		"recordAppliedMigration, %s -> apply-ledger-unwritable naming the row, generic branch (no 23502 sentence)",
		async (_label, error) => {
			const { session } = makeScriptedSession([
				{ throws: error },
				{ rows: [{ currentUser: "ld_noinsert" }] },
			]);
			const tag = await caught(
				recordAppliedMigration(session, "0001_init.sql", "applied"),
			);

			await expect(
				throwLedgerWriteFailure(session, tag, Command, "0001_init.sql"),
			).rejects.toSatisfy((thrown: unknown) => {
				if (!(thrown instanceof HejbroError)) {
					return false;
				}
	...
	it("recordAppliedMigration, 23502 with .column -> the identity/default sentence, naming the column", async () => {
	...
	it("clearLedgerRows, 42501 -> apply-ledger-unwritable naming the clearing, drops-rolled-back sentence", async () => {
		const error = permissionDeniedTable();
		const { session } = makeScriptedSession([
			{ throws: error },
			{ rows: [{ currentUser: "ld_noinsert" }] },
		]);
		const tag = await caught(clearLedgerRows(session));

		await expect(
			throwLedgerWriteFailure(session, tag, "hejbro reset"),
		).rejects.toSatisfy((thrown: unknown) => {
			if (!(thrown instanceof HejbroError)) {
				return false;
			}
