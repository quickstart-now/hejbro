...
| D73 | **`formatVersion` tracks field shapes, not vocabulary — core kind additions don't bump it, before or after publication.** #23's `sequence` kind (Phase 8, `sequence-kind`) lands without moving `formatVersion` (D68 precedent: adding `supabase-storage-bucket` in Phase 6 didn't bump it either). A stale reader given a snapshot from a newer hejbro therefore doesn't fail at `parseSnapshot` (the version number still matches) — it fails inside diffing, the first time `KindRegistry.get` is asked for a kind id it never registered (`unknown-kind`). That diagnostic now names both causes and picks between them by the id's shape: no namespace prefix (`sequence`, `table`, ...) reads as core's own still-growing vocabulary and points at upgrading hejbro; a namespace-prefixed id (`supabase-storage-bucket`) reads as a preset's and keeps the original "register the preset" advice — `register()`'s own `duplicate-kind` message already tells preset authors to prefix, so the shape is real signal, not a guess invented for this diagnostic. Checking this at `parseSnapshot` time instead was considered and set aside: `parseSnapshot` takes no `KindRegistry`, and every real call site (`packages/cli`'s `generate.ts`, `verify.ts`) builds its registry *after* calling `parseSnapshot`, not before — moving the check there needs both a widened signature and reordering three call sites, out of scope for this fix (decided 2026-08-22, Phase 8 #23 review; owner-approved) | bump `formatVersion` for every core kind added (a "kind counter"); leave `unknown-kind` as it was, expecting `formatVersion` to catch a too-new snapshot before the message is ever reached | Measured both directions, not assumed: reading a real `sequence`-carrying snapshot (same `formatVersion: 5`) through pre-`sequence-kind` core reproduced exactly this — `parseSnapshot: OK`, then `unknown-kind` with the wrong advice (#193 review, #196's origin) — so "formatVersion protects against this" was falsified, not just untested. A kind counter would fix the symptom but re-opens exactly what D68 closed: whether adding a kind is a format change, this time per-kind instead of per-wave, forcing a bump on every future core kind indefinitely. Not bumping v5 for `sequence-kind` itself is D68's own already-decided scope (a new kind needs no bump of its own) — this decision is about what stands in for that bump's absence at the point a stale reader actually meets the gap, not about reopening whether to bump |
