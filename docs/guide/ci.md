...
`hejbro verify` exits `0` when every check passes, `1` if any fails. Five checks always run; two more join independently when they apply — an export-freshness check (once `generate --export` is in use) and a check for every registered preset's own validators (once the active config registers at least one preset validator) — so the total is five to seven. A failure prints every failing check as a diagnostic block plus a summary line:
