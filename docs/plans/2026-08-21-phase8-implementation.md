...
| 31 | `phase8-crap-ratchet-5` | `CRAP_THRESHOLD = 5`. **Measured 2026-08-22 at `dev@8261b88` with the threshold set to 5: 42 functions, not the 29 this row used to say** — 36 at complexity 6–10 (a split is the only way down: at 100% coverage CRAP equals complexity) spread over core kinds, engine, expr/dsl/plpgsql/snapshot and four supabase functions, plus 6 at complexity 3–5 that coverage alone clears. Too large for one PR: four or five — core kinds · core engine · core expr/dsl/plpgsql/snapshot · supabase + the six coverage-only · the threshold flip last — each with zero behaviour change and red-first proof, ordered so the hot files of the open A/C PRs land first. Track B | #154 | — |
...
| Track | Area | Queue |
|---|---|---|
| A | `packages/core/src/expr`, `packages/core/src/kinds`, core semantics, `packages/cli` commands | ~~`expr-nodes` → `view-nodes` → `sequence-kind` → `chain-walk`~~ ~~→ `duplicate-version` → `constraint-names`~~ → `verify-fix` (in progress; the duplicate-version message and `--fix` share one chain-order plan; a group whose order the chain cannot decide gets a pick-one list of concrete `mv` commands and `--fix` skips it) → `grant-sync` → `scope-nodes` (the row-48 walker is on `dev` since #236) |
| B | `.github/workflows/`, `scripts/`, `docs/`, tooling, validators that touch no hot file | ~~`release-workflows` → `crap-tooling` → `roadmap-sync` → `cli-timeout` → `supabase-image` → `docs-release` → `diagnostic-xref` → `policy-schema-usage`~~ ~~→ `fresh-build` → `naming-convention` → `crap-gate` → `clause-context`~~ → `crap-ratchet-5` (four or five PRs, see row 31; the supabase + coverage-only slice first, it overlaps no open PR) → `oidc-publishing` (branch only until the publish) (draft now, apply after 0.1.0 with the owner) |
| C | `examples/`, golden content, `packages/supabase`, **and since 2026-08-22 the CRAP work and snapshot validation** | ~~`golden-english` → `bucket-notes` → `preset-goldens` → `crap-refactor` → `declared-vs-catalog` → `crap-walkers` → the CRAP tail~~ (`dev` reports 0 at threshold 10 since `836fa7b`; the gate is in CI since `437a9c3`) ~~→ `drift-if-exists`~~ → `history-restore` (spec final; `history` done, `restore` in progress) → `banner-version` (`constraint-names` merged — ready) (after `constraint-names` merges) → `crap-coverage` → `catalog-types` → `required-keys` → `crap-ratchet-5` (after `crap-gate`; last) |
