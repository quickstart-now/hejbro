...
overlap — rows run 1–50 and issues run #9–#229, so `22` and `#22` are different
...
| 31 | `phase8-crap-ratchet-5` | `CRAP_THRESHOLD = 5`. **Measured 2026-08-22 at `dev@8261b88` with the threshold set to 5: 42 functions, not the 29 this row used to say** — 36 at complexity 6–10 (a split is the only way down: at 100% coverage CRAP equals complexity) spread over core kinds, engine, expr/dsl/plpgsql/snapshot and five supabase functions, plus 6 at complexity 3–5 that coverage alone clears. Too large for one PR: four or five — core kinds · core engine · core expr/dsl/plpgsql/snapshot · supabase + the six coverage-only · the threshold flip last — each with zero behaviour change and red-first proof, ordered so the hot files of the open A/C PRs land first. Track B (③-b `dsl/*` on A). **Landed: ④ (#241, 42→32), ③-a (#242, 35→25 at `8b22258`) and ③-b (#247, 26→23, `dev@b66c122`); ② is two PRs — ②-a (#249, merged at `dev@869376c`: 12 of the engine's 13 functions incl. `createOrDropDiff` shared by eight kinds and the three in `duplicate-version-fix.ts`, 25→13 on its base) and ②-b (#253, merged at `dev@ebea52a`: `generateMigration` as options-defaults resolver · blocked-result early return · predrop sort helper · a thin body, plus the two inflow functions below, 12→9; its review found that the predrop rank order — #122, a dependent kind drops first — had no test pinning it, so one was added with a mutation proof, and the identity tiebreak inside the same sort was shown unreachable because `diffSnapshots` already orders within a kind — pinned, not mutation-tested) — and ① is two PRs by two implementers, ①-A (#252, merged at `dev@fa49e8f`: `table-kind*.ts`, `policy-kind.ts` — policy emit through `dispatchEmit`, the table diff in three, `sequenceForAddedColumn` predicates, 12→9) and ①-B (#254: rls/sequence/schema/grant/trigger emit as handler maps, `sequence-kind.diff` on `createOrDropDiff`, 6→0, and the flip — merged at `dev@b2776c4`, which closed #154), with ⑤ the flip to `CRAP_THRESHOLD = 5` as the **last commit of whichever ① PR merges last** (it was #254's second commit; CI ran `check:crap` at 5 on that head and reported 0 of 924 functions), and that PR's freeze condition is *merge-preview against `dev` measures 0 at 5* — measuring each slice against current `dev` is what caught two functions that arrived between measurements (`scopeViolationHandlers.sqlTemplate` from #245, `validateRequiredKeys` from #248); both ride with ②-b. Reviewer rule from ④/③-a: a `.some()`/`.every(Boolean)` rewrite of a plain `&&`/`\|\|` chain is a metric dodge, not a split — helper extraction yes, array tricks no, and the comment says what the guard answers, never what the walker counts** | #154 | — |
...
| 45 | `phase8-history-restore` | `hejbro history` / `hejbro restore <n>` (owner's design memo in the issue): migration-number ↔ commit correspondence through the snapshot hash in the banner, declaration files restored from the target commit, dirty-tree refusal, no TTY prompts (D32), git only in `packages/cli`. **Design first** (designer: CLI surface, byte-stable non-TTY output, link rules; researcher: the four open questions) → owner decisions, then code. Depends on #129 (landed) and row 42 **Decided 2026-08-22 (owner): candidate = the commit that added the file (`git log --diff-filter=A`), proof = the banner's `snapshot:` hash equals the sha256 of that commit's snapshot *blob* (never the disk file — CRLF checkouts; never re-parsed — old format versions throw), full-history search on mismatch, `lost`/`rewritten`/`uncommitted`/`ok` states in `history`, `restore` refuses `lost` and `rewritten` with the closest available state named; position-based pairing is dropped; no extra bookkeeping; `--first-parent` never. The squash-merge limit is stated only by `history`/`restore` output and the §7 messages — no document or skill recommends "one migration per PR" or any workflow. `--links`/`--no-links` and the exit matrix (history fails only outside git; restore fails on anything short of its goal; an uncommitted target is a no-op) are fixed; the restore scope is the entry-glob files aligned to the target commit's set (files that exist only now are removed), each file action is printed diff-style (`+ restored` / `~ restored` / `- removed`, coloured only on a TTY) followed by the computed undo commands, and after restoring, the declarations are loaded and re-rendered: their snapshot hash must equal the banner's (`restore-state-mismatch` names the files outside the entry glob that changed since the commit); a migration that predates the current snapshot format skips that check with a note. No new flags for any of it — the output explains. Spec: the designer's final document, recorded on the issue; implementer-3 after #198** | #130 | — |
| 46 | `phase8-fresh-build` | Running `vitest` inside one package bypasses turbo's `^build` and asserts against whatever `dist` exists (a silently false green). Measured 2026-08-22: `pretest` hooks only script-name calls (`pnpm test`, `pnpm --filter x test`) and misses `vitest`/`pnpm exec vitest`, the path the issue names; with a stale `dist` a core regression passed supabase's 86 tests. **Decided: in-process tests resolve `@hejbro/core` to `packages/core/src/index.ts` (public index only — the preset boundary holds; packaging is `smoke:pack-install`'s job), and the tests that run the built CLI as a subprocess get a freshness guard that fails with `Next: run pnpm build` when `dist` is older than `src` — detection, not prevention.** Scope adds `packages/cli` (same exposure). In review as `phase8-fresh-core-tests`: the guard's `Next` says `pnpm build --force`, because a turbo cache hit replays logs without rewriting `dist` | #131 | — |
| 47 | `phase8-naming-convention` | Biome `useNamingConvention` for hejbro's own TypeScript: the 62 warnings measured and categorised (genuine / SQL-mirroring fixtures under `assertSqlName` / other), then enabled. **Measured 2026-08-22: 64, of which 61 are Biome's `strictCase` reading `T`-prefixed generics (`TFamily`, …) and `LeavesASurvivor` as consecutive capitals — `options.strictCase: false` clears them globally; the 3 real SQL/env fixtures (`NO_COLOR`, `foo_bar`, `post_id`, D36) get a line-level `biome-ignore` with the reason rather than a path override, which would hide other findings in those files; no identifier is renamed; the rule flips to `error` in the last commit, with a planted violation as the positive control** | #132 | — |
| 48 | `phase8-clause-context` | `design:` an `Expr` does not know which clause it sits in, so `authUidCached()`'s `(select auth.uid())` cannot be refused outside RLS. Researcher compares: a marker outside the snapshot (the `Symbol.for` family, D33/D67/D70 intact) checked at each clause entry, render-time checking, or documenting the gap — and the shape row 37 (#160) should share. **Owner decision: does a clause taxonomy belong in core's extension interface?** Code only after that **Decided 2026-08-22 (owner): no** — core exports one deep expression walker (into `exists()`), the supabase preset refuses its clause-restricted helpers inside a column default / CHECK / index predicate with its own validator (generate-time error, one golden), `sql.raw` is the documented escape hatch, and the marker design is re-asked when a second preset needs the same helper (D76). #160 builds on the same walker | #141 | — |
| 49 | `phase8-drift-if-exists` | `design:` `policy`/`trigger` drop with `if exists`, which swallows out-of-band removal. Researcher **measures** whether `verify`, the pg_dump round-trip, or row 39's catalog check already detect that drift (Docker), then the owner chooses: keep `if exists` and record the detector in the spec, or the hybrid (suppress the drop only when the owner object is gone). Sequences already drop plainly on `predrop` **Measured and decided 2026-08-22 (owner): `verify` never reads a database (D33), the round-trip compares two harness databases, only row 39's check saw the drift; the hybrid is unreachable (dependents drop on `predrop`, the owner table always exists); the create path's `if exists` cannot hide drift at all. So: alter/drop go bare like `sequence`, create keeps `drop … if exists`, a D75 row says drift while declarations are unchanged is `supabase db diff`'s job — no hejbro catalog tool, no issue for one** | #198 | — |
| 50 | `phase8-banner-version` | The migration banner gains `-- hejbro: <version>` (owner, 2026-08-22): core's `renderBanner` emits the line from a string the CLI injects from its own `package.json` (core stays pure); `verify`'s banner parser keeps reading only the known prefixes (older files without the line stay valid) and exposes the version; goldens regenerate once in their own commit; committed example steps are history, new steps carry the line. Lets #130's `restore-state-mismatch` say "generated by hejbro 0.1.0, this build is 0.2.0". Track C, after #198 and **after `phase8-constraint-names` merges** so the goldens regenerate once for both | #229 | — |
...
| Track | Area | Queue |
|---|---|---|
| A | `packages/core/src/expr`, `packages/core/src/kinds`, core semantics, `packages/cli` commands | ~~`expr-nodes` → `view-nodes` → `sequence-kind` → `chain-walk` → `duplicate-version` → `constraint-names` → `verify-fix` → `grant-sync` → `scope-nodes`~~ ~~→ `crap-ratchet-5` slice ③-b~~ ~~→ `crap-ratchet-5` slice ①-A~~ (#252) — **A is done** |
| B | `.github/workflows/`, `scripts/`, `docs/`, tooling, validators that touch no hot file | ~~`release-workflows` → `crap-tooling` → `roadmap-sync` → `cli-timeout` → `supabase-image` → `docs-release` → `diagnostic-xref` → `policy-schema-usage`~~ ~~→ `fresh-build` → `naming-convention` → `crap-gate` → `clause-context`~~ → `crap-ratchet-5` (row 31: ~~④ supabase + coverage-only~~ · ~~③-a expr/plpgsql/snapshot~~ · ~~②-a engine~~ (#249) · ~~②-b `generateMigration` + inflow~~ (#253) · ~~①-B the kinds ①-A does not take · ⑤ the threshold flip in the last ① PR~~ (#254)) → `oidc-publishing` (branch `phase8-oidc-publishing` ready; applied after 0.1.0 with the owner) |
| C | `examples/`, golden content, `packages/supabase`, **and since 2026-08-22 the CRAP work and snapshot validation** | ~~`golden-english` → `bucket-notes` → `preset-goldens` → `crap-refactor` → `declared-vs-catalog` → `crap-walkers` → the CRAP tail~~ (`dev` reports 0 at threshold 10 since `836fa7b`; the gate is in CI since `437a9c3`) ~~→ `drift-if-exists` → `history-restore` → `banner-version` → `catalog-types`~~ ~~→ `required-keys`~~ (#248, #159 closed) ~~→ the `catalog-types` follow-up~~ (#250: cast-suffix character class, expression-default limit in the header, and quoted negative numeric literals) — **C is done**; `crap-coverage` dissolved into the ratchet slices; `crap-ratchet-5` is B's |
...
Three more instances on 2026-08-22 sharpened two corollaries. (1) **A freeze
needs the PR's own CI green, not only the local gates** — `phase8-history-
restore` was frozen on 11/11 local tasks while `verify (22)/(24)` were red on
`check:next-marker`, a step the local run had not included; the freeze line now
carries the CI run id and its head SHA, and the local gate list includes every
CI step (`check:next-marker`, `check:diagnostic-xref`,
`check:first-release-version`). (2) **After a freeze or a merge request, nobody
pushes to that branch — not for a rebase, not for a fix the planner asked for.**
`phase8-plan-rules-14` (planner's nit fix), `phase8-grant-sync` (a banner line
the planner had asked to verify) and `phase8-history-restore` (a rebase onto a
`dev` that had moved) were each pushed after their freeze; the guard refused
