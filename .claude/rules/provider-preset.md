...
A provider preset package (`@hejbro/supabase`, `@hejbro/neon`) may only
use `@hejbro/core`'s public extension interface (spec §4.1) plus, since
...
- `@hejbro/supabase` was the template this rule generalized from;
  `@hejbro/neon` is the first package that tested the generalization.
  Anything done in either must keep generalizing — the next preset (Nile)
