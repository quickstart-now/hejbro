...
these objects, or adding the missing column in a following edit for a
database that lacks one. The migration is still written: this is a
notice, not a refusal. A handover, and a table that is new outright,
print nothing under this code.
...
- **THEN** the run prints `adoption-creates` for `widgets` naming the
  ...
  missing-column risk sentence and a `Next:` naming both `hejbro
  baseline` and adding a missing column in a following edit, and the
