...
## Re-verify before transcribing (task 5.3)

The row quotes three measured figures: **ten** mapped-type registries,
**9 of 13** goldens, **24** banner hash lines. They were measured after
the #444 rebase. The final rebase pulls in more commits, and if any of
them adds or retires an `ExprNode["nodeKind"]` registry the first figure
stops being true of the tree the row describes. Re-count before writing
the row; if a figure moved, the corrected wording goes back to the lead
before it lands, because the approval was of a specific claim, not of a
placeholder.

(Expected outcome: unchanged. #456 and #457 touch gate scripts, #459
touches docs, and the archive batch touches `openspec/`/`blackbox/` —
none of them is an expression registry. "Expected" is why it is checked
rather than assumed.)
