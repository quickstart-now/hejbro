...
with no `RETURNING` clause produces none, and Postgres rejects such a
body when the function is created, so nothing is gained by deferring the
refusal to apply time.
