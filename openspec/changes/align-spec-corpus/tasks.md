...
- [ ] 1.1 `[design]` ~10m Confirm the on-conflict delta against the
...
- [ ] 1.2 ~8m Bind the split query-execution requirements' scenarios
...
- [ ] 1.3 `[design]` ~5m Confirm the IntervalValue delta against the
...
- [ ] 1.4 ~8m Verify the chain stage-parity list against
...
- [ ] 2.1 ~10m Bind the `generate`/`verify` seed scenarios
...
- [ ] 2.2 ~8m Bind the `diagnostics` and `migration-format` seed
...
- [ ] 2.3 ~10m Write the failing `@hejbro/neon` README mode-mismatch
...
- [ ] 3.1 ~6m Add the P6 recurrence rule to `openspec/config.yaml`
...
- [ ] 3.2 ~5m Refresh the two stale Purpose sections in the MAIN specs
...
- [ ] 3.3 ~8m Write the `blackbox/` entry for this owner-driven change
